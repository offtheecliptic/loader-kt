...
fun loadExtension(properties: Map<String,String>,
    ...
    if (extensionRootURL != null) {
        ...
        val classLoader: PostDelegationClassLoader 
            = PostDelegationClassLoader(classpath as Array<URL?>?, 
                                        Extension::class.java.classLoader) 
        
        // Now use the post-delegation classloader to instantiate the extension in 
        // the classloader.  This is done by reflection in the classloader, so there
        // is no need for this main app to have the extensionImpl class at compile time, 
        // although it must have the extension interface.

        // The 'extension' instance is actually a extension impl, which implements 
        // the extension interface. The ExtensionImpl class will be loaded in the 
        // extension-specific class loader, then instantiated.
        ...
        // Initialize extension; every extension has an init functiom that takes properties
        extension.init(properties)

        // Use the extension, which will use a different version of ComponentImpl
        // Because the secondary CL was used to load the 2 classes in extension.kt, 
        // we get ComponentImpl v2.
...
/**
 * Helper function to compute the plugin root directory URL given the plugin 
 * root directory, and an example (in that root directory) class file's fully-
 * qualified class name.
 *
 * The example class file is only provided to verify that the directory is the 
 * right one.
 *
 * Params:
 *     fileDir   - Root directory under which all plugins live
 *     className - Fully-qualified class name of an exemplar plug impl file, 
 *                   to verify that the file root does indeed hold plugins
 */
